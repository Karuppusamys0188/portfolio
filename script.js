function generateCourseInputs() {
  ...
  if (numCourses < 1) {
    document.getElementById("error").innerText =
      "Please enter a valid number of courses (at least 1).";
  ...
  for (let i = 1; i <= numCourses; i++) {
    ...
    courseRow.innerHTML = `
            <td><input type="number" step="0.01" class="input-field" placeholder="Credits" id="credits${i}" min="0"></td>
            <td><input type="text" class="input-field" placeholder="Grade" list="inputlist" id="grade${i}"></td><datalist id="inputlist">
                <select>
...
function calculateGPA() {
  ...
  for (let i = 1; i <= numCourses; i++) {
    ...
    if (isNaN(credits) || credits <= 0) {
      ...
      document.getElementById(
        "error"
      ).innerText = `Invalid credits entered for Course ${i}. Please enter a valid positive number.`;
      document.getElementById("error").style.display = "block";
      return;
    }
    if (grade in gradePoints) {
      ...
      document.getElementById(
        "error"
      ).innerText = `Invalid grade entered for Course ${i}. Please enter a valid grade (S, A, B, C, D, E, F).`;
      document.getElementById("error").style.display = "block";
      return;
    }
  }
  ...
  if(gpa == 10){
    message = `🎉 Contact Me  (I need some tips) 🎉`;
  } else if (gpa >= 9) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const toggle_btn = document.querySelector(".round");
  toggle_btn.addEventListener("click", (event) => {
    event.preventDefault();
    body.classList.toggle("black");
    container.classList.toggle("white");
    developer.classList.toggle("white");
    toggle_btn.classList.toggle("up");
    toggle_btn.classList.toggle("down");
  });
